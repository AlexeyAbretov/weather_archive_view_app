...
export function parseAnchorDate(date: Dayjs): AnchorDate {
  return {
    month: date.month() + 1,
    day: date.date(),
    year: date.year(),
...
export function toDayjs(anchor: AnchorDate): Dayjs {
  return dayjs()
    .year(anchor.year)
...
export function formatAnchorDate(anchor: AnchorDate): string {
  return toDayjs(anchor).format('D MMMM YYYY');
}

export function getAnchorYear(anchor: AnchorDate): number {
  return anchor.year;
}

/** Диапазон лет anchorYear − 10 … anchorYear + 10 (21 год). */
export function getYearRange(anchor: AnchorDate): number[] {
...
export function formatYearRange(anchor: AnchorDate): string {
